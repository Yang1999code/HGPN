...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads):
        ...
        # self.feature_projection = nn.Linear(2, 64)  # 从特征维度3到64
        # 
        ...
        self.d_model = d_model  # 这里存储d_model的值
        self.feature_projection = nn.Linear(2, 16)  # 从特征维度3到64
        self.inner_attention = attention
        self.n_heads = n_heads
        self.head_dim = d_model // n_heads
...
#     def __init__(self, attention, d_model, n_heads):
#         super(AttentionLayer, self).__init__()
#         self.feature_projection = nn.Linear(2, 256)  # 将特征维度从3转换为d_model
#         self.inner_attention = attention
...
#
#     def forward(self, x, attn_mask=None):
#         # print(f"Input x shape before feature projection: {x.shape}")
#         x = self.feature_projection(x)  # 将输入特征维度从3转换为d_model
#         # print(f"Input x shape after feature projection: {x.shape}")
#
#         queries = self.query_projection(x).view(x.shape[0], -1, self.n_heads, self.head_dim)
#         keys = self.key_projection(x).view(x.shape[0], -1, self.n_heads, self.head_dim)
#         values = self.value_projection(x).view(x.shape[0], -1, self.n_heads, self.head_dim)
...
class BackboneODE(nn.Module):
    ...
    def __init__(self, lookback, feature_dim, ode_hid_dim, method, ffcm_dim, d_model, n_heads, window_size, dropout):
        ...
        # self.project_to_256_self = nn.Linear(feature_dim, 2)
        # self.project_to_256_attention = nn.Linear(256, 256)
        #
        # self.ffcm = FFCM(dim=ffcm_dim, token_mixer_for_gloal=Freq_Fusion)
        # self.project_to_256_ffcm = nn.Linear(128, 256)
        # self.project_to_256_self = nn.Linear(feature_dim, 2)
        # self.project_to_256_attention = nn.Linear(64, 64)
        # self.ffcm = FFCM(dim=ffcm_dim, token_mixer_for_gloal=Freq_Fusion)
        # self.project_to_256_ffcm = nn.Linear(128, 64)

        self.project_to_256_self = nn.Linear(feature_dim, 2)
        self.project_to_256_attention = nn.Linear(16, 16)
        self.ffcm = FFCM(dim=ffcm_dim, token_mixer_for_gloal=Freq_Fusion)
        self.project_to_256_ffcm = nn.Linear(128, 16)
        ...
        # self.attention_projection = nn.Linear(64, 2)  # 将 attention 输出从 256 维投影到 2 维
        # self.ffcm_projection = nn.Linear(64, 2)  # 将 ffcm 输出从 256 维投影到 2 维
        self.attention_projection = nn.Linear(16, 2)  # 将 attention 输出从 256 维投影到 2 维
        self.ffcm_projection = nn.Linear(16, 2)  # 将 ffcm 输出从 256 维投影到 2 维

    # def dxdt(self, t, x):
    #     x_self = self.f(x)
    #     x_neigh = self.g(x)
    #     x_attention = self.attention_layer(x)
    #     x_attention =self.project_to_256_attention(x_attention)  # 已经是256维度，无需变换
    #
    #     x_ffcm = self.ffcm(x.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1)
    #     x_ffcm = self.project_to_256_ffcm(x_ffcm)  # 变换到256维度
    #     print("x_self shape:", x_self.shape)
    #     print("x_neigh shape:", x_neigh.shape)
    #     print("x_attention shape:", x_attention.shape)
    #     print("x_ffcm shape:", x_ffcm.shape)
    #     dxdt = x_self + x_neigh + x_attention + x_ffcm
    #     return dxdt
    def dxdt(self, t, x):
        # 计算 x_self 和 x_neigh
        x_self = self.f(x)  # x_self 输出 (batch_size, 77, 2)
        x_neigh = self.g(x)  # x_neigh 输出 (batch_size, 77, 2)

        # 处理 attention 和 ffcm 输出
        x_attention = self.attention_layer(x)  # 输出 (batch_size, 77, 256)
        # print("x_attention shape (before projection):", x_attention.shape)

        # 确保 ffcm 输出为 (batch_size, 77, 128)（根据之前的报错）
        x_ffcm = self.ffcm(x.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1)

        # 打印 x_ffcm 形状
        # print("x_ffcm shape (before projection):", x_ffcm.shape)

        # 如果 x_ffcm 是 128维，确保它经过适当的线性层转换到 256维
        if x_ffcm.shape[-1] == 128:
            x_ffcm = self.project_to_256_ffcm(x_ffcm)  # 更新至 256维
            # print("x_ffcm shape (after projection):", x_ffcm.shape)

        # 将 x_attention 和 x_ffcm 通过线性层统一为 2 维
        x_attention_projected = self.attention_projection(x_attention)  # 将 (batch_size, 77, 256) 变为 (batch_size, 77, 2)
        x_ffcm_projected = self.ffcm_projection(x_ffcm)  # 将 (batch_size, 77, 256) 变为 (batch_size, 77, 2)

        # 确保 x_self 和 x_neigh 也是 2 维
        x_self_projected = self.project_to_256_self(x_self)  # 变为 (batch_size, 77, 2)
        x_neigh_projected = self.project_to_256_self(x_neigh)  # 变为 (batch_size, 77, 2)

        # 打印输出形状以调试
        # print("x_self shape:", x_self_projected.shape)
        # print("x_neigh shape:", x_neigh_projected.shape)
        # print("x_attention shape:", x_attention_projected.shape)
        # print("x_ffcm shape (after projection):", x_ffcm_projected.shape)

        # 最后进行加法
        dxdt = x_self_projected + x_neigh_projected + x_attention_projected + x_ffcm_projected
        # dxdt = x_self_projected + x_neigh_projected
        return dxdt
